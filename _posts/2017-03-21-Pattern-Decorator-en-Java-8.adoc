...
Pour résumer, l'idée de ce pattern est d'ajouter du comportement à du code au runtime (c'est à dire à l'exécution du code).
Ce pattern présente de nombreux avantages car lorsque la conception est mise en place, il devient très simple d'ajouter du comportement au code existant, ce qui le rend très évolutif.
