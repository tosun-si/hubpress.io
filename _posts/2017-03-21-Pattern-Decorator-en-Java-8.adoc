...
Pour résumer, l'idée de ce pattern est d'ajouter du comportement au code lors du runtime (c'est-à-dire à l'exécution du code).
...
Dans notre cas une méthode finale permet d'exécuter la fonction globale à la classe avec la valeur globale "function.apply(value)". Ainsi le résultat de la fonction est récupéré, c'est à dire le calcul du bénéfice : 
